...
func runUpdateNotices() {
	...
	if err := cmd.Run(); err != nil {
		verbose.Debug("liferay update release-notes error:", err)
		return
	}
...
func notify(c *config.Config) {
	...
	_, _ = fmt.Fprintln(os.Stderr, color.Format(color.FgBlue,
		`
INFO: New version of Liferay CLI is available. Please run "%v".`,
		cmd))
}
...
func handleUpdateApplyError(err error) error {
	if err != nil && strings.Contains(err.Error(), "permission denied") {
		return errwrap.Wrapf(`permission denied. Try "sudo liferay update" instead`, err)
	}
