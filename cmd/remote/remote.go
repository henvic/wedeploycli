...
// RemoteCmd is the command to control Liferay remotes.
var RemoteCmd = &cobra.Command{
	Use:    "remote",
	Hidden: true,
	Short:  "Configure Liferay remotes",
	Args:   cobra.NoArgs,
	RunE:   remoteRun,
}
...
func removeRun(cmd *cobra.Command, args []string) error {
	...
	if name == defaults.CloudRemote {
		_, _ = fmt.Fprintf(os.Stderr, "%v\n", color.Format(color.FgHiRed, `Removed default cloud remote "liferay" will be recreated with its default value`))
	}

	if name == params.DefaultRemote && name != defaults.CloudRemote {
		params.DefaultRemote = defaults.CloudRemote
		conf.SetParams(params)
		_, _ = fmt.Fprintf(os.Stderr, "%v\n", color.Format(color.FgHiRed, `Default remote reset to "liferay"`))
	}
